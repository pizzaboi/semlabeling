...
class Evaluater:
    ...
    def checker(self):
        """Check if two sequences matches or not and count # of match"""
        if self.system: self.unitcount += 1
...
def evaluate_f(bi=False):
    Precision = Evaluater()
    Recall = Evaluater()
    #for line in open(src):
    for line in sys.stdin:
        line = line.strip('\n')
        if line.startswith('O'): continue # Don't care if gold label is 'O'
        if not line:
            Precision.checker()
            Recall.checker()
        else:
...
def evaluate_accuracy(src):
    ...
    Matrix.result()
    Sentence.result()
    print 'MATRIX ACCURACY: {} SENTENCE ACCURACY: {}'.format(Matrix, Sentence)
...
if __name__ == '__main__':
    ...
    #test('work/rulebased_ver2.0.eval', 'ラベルver2.0のルールベース解析結果')
